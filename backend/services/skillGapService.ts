...
export interface SkillGapAnalysisResult {
  ...
  current_strengths: CurrentStrength[];
...
const DOMAIN_REQUIREMENTS: Record<string, DomainRequirements> = {
  AI: {
 ...
 */
async function generateAISkillGapAnalysis(
  profile: SkillGapProfile
): Promise<SkillGapAnalysisResult> {
  if (!openai)
...
- Include free resources whenever possible`;
  ...
  const parsed: SkillGapAnalysisResult = JSON.parse(cleaned);
...
function generateRuleBasedAnalysis(
  ...
  const currentStrengths: CurrentStrength[] = matched.map((skill) => ({
    skill,
  ...
  const skillGaps: SkillGapItem[] = missing.map((skill) => {
...
export { analyzeSkillGaps, generateRuleBasedAnalysis };
