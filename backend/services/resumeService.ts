...
import { extractSkillsFromResume, analyzeResume } from "./openaiService";
import type { ExtractedResumeProfile, ResumeAnalysisResult } from "./openaiService";
...
export interface FullResumeAnalysis {
  extractedProfile: ResumeProfile;
...
type SectionPatterns = Record<string, RegExp>;
...
const ALL_SKILLS: string[] = Object.values(SKILL_DATABASE).flat();

// Section headers that indicate different resume sections
const SECTION_PATTERNS: SectionPatterns = {
 ...
 */
function detectSections(text: string): ResumeSections {
 ...
 */
function extractProfileFromResume(text: string): ResumeProfile {
  if (!text || typeof text !== "string") {
    return { skills: [], interests: [], experience_level: "beginner", preferred_domain: "all", sections: [] };
  }
  ...
  for (const skill of ALL_SKILLS) {
    ...
    if (pattern.test(lowerText)) {
      let weight = 1;
      if (pattern.test(skillSection)) weight += 2;
      if (pattern.test(projectSection)) weight += 1;
 ...
 */
function computeResumeCompleteness(
  ...
  let score = 0;
  if ((sections.education && sections.education.length > 0) || education.length > 0) score += 15;
  if (sections.experience && sections.experience.length > 0) score += 25;
  if (sections.skills && sections.skills.length > 0) score += 15;
  if (sections.projects && sections.projects.length > 0) score += 15;
 ...
 */
async function extractProfileWithAI(resumeText: string): Promise<ResumeProfile> {
  const keywordProfile = extractProfileFromResume(resumeText);

  try {
    const aiProfile: ExtractedResumeProfile = await extractSkillsFromResume(resumeText);

    // Merge skills — union of AI and keyword-based, deduplicated
 ...
 */
async function getFullResumeAnalysis(resumeText: string): Promise<FullResumeAnalysis> {
  const extractedProfile = await extractProfileWithAI(resumeText);

  let analysis: ResumeAnalysisResult | null;
  try {
    analysis = await analyzeResume(resumeText, extractedProfile.skills);
  } catch (error: unknown) {
    const errMsg = error instanceof Error ? error.message : String(error);
    logger.warn("Resume analysis failed", { error: errMsg });
    analysis = null;
  }
...
export { extractProfileFromResume, extractProfileWithAI, getFullResumeAnalysis };
